...
%       of "tree" and "Bank".
%-------------------------------------------------------------------------------------
function [MOVIESID,tree,root,Bank,AVAIL_tree,AVAIL_Bank]=New_Divide_Children(MOVIESID,ID,tree,Bank,root,bf,gh,parent_node,AVAIL_tree,AVAIL_Bank)
D=size(tree);
m=size(Bank);
d=(D(2)-3)/2;

%--------------------------------------------------------------------------
Hdistans=zeros(1,D(2)-3);
for i=1:D(2)-3
    A=Bank(tree(tree(parent_node,i+3),2),2:m(2));
    Hdistans(1,i)=Hamming_Distance_new(A,bf);
end %end of for
[~,I]=sort(Hdistans);
...
KK=tree(parent_node,4:D(2));
for j=1:d
    tree(new_parent,j+3)=KK(I(j)); %tree(parent_node,d+4:D(2));
    tree(parent_node,j+3)=KK(I(j+d));
end %end of for
...
Bank(dd,1)=-2;
%--------------------------------------------------------------------------
distance1=Hamming_Distance_new(bf,newparent1);
distance2=Hamming_Distance_new(bf,newparent2);
...
if root==parent_node
    ...
    if tree(tree(parent_node,3),D(2))~= 0
        ...
        [MOVIESID,tree,root,Bank,AVAIL_tree,AVAIL_Bank]=New_Divide_Children(MOVIESID,ID,tree,Bank,root,bf,gh,tree(parent_node,3),AVAIL_tree,AVAIL_Bank);
